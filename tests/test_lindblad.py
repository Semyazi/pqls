...
def generate_random_system(n_levels: int, num_c_ops: int, seed: int):
    ...
    # Random Hermitian Hamiltonian: H = 0.5 * (A + A^dag)
...
def test_raw_solve_steady_state_vs_qutip(
    ...
    h, c_ops = generate_random_system(
        n_levels=n_levels, num_c_ops=n_levels - 1, seed=seed
    )
...
def test_solve_steady_state_direct_batch_vs_qutip(
    ...
    a = rng.normal(size=(batch_size, n_levels, n_levels)) + 1j * rng.normal(
        size=(batch_size, n_levels, n_levels)
    )
    h_batch = 0.5 * (
        a + np.swapaxes(a.conj(), -1, -2)
    )  # Shape (batch_size, n_levels, n_levels)
...
def test_ladder_system_vs_qutip(
    n_levels: int, batch_size: int, seed: int, tol_parity: float
):
    ...
    detunings = [
        rng.uniform(-20e6, 20e6, size=batch_size)
        if batch_size > 1
        else float(rng.uniform(-20e6, 20e6))
        for _ in range(num_transitions)
    ]
    rabis = [
        rng.uniform(1e5, 5e6, size=batch_size)
        if batch_size > 1
    ...
    dets_mat = np.stack(all_bcast[:num_transitions], axis=-1).reshape(
        n_points, num_transitions
    )
    rabs_mat = np.stack(all_bcast[num_transitions:], axis=-1).reshape(
        n_points, num_transitions
    )

    qt_coherences = [
        np.imag(
            solve_qutip_steady_state(
                build_ladder_hamiltonian(dets_mat[k], rabs_mat[k]), c_ops
            )[1, 0]
        )
...
def test_solve_quantum_ladder_validation_and_e2e(tol_parity: float):
    ...
    gammas = [
        ...
        1.0
        / atom.getStateLifetime(50, 2, 2.5, temperature=300.0, includeLevelsUpTo=80),
        1.0
        / atom.getStateLifetime(51, 1, 1.5, temperature=300.0, includeLevelsUpTo=80),
    ]

    dipoles = [
        au_to_coulomb_metre(
            abs(atom.getDipoleMatrixElement(5, 0, 0.5, 0.5, 5, 1, 1.5, 1.5, 1))
        ),
        au_to_coulomb_metre(
            abs(atom.getDipoleMatrixElement(5, 1, 1.5, 1.5, 50, 2, 2.5, 2.5, 1))
        ),
        au_to_coulomb_metre(
            abs(atom.getDipoleMatrixElement(50, 2, 2.5, 2.5, 51, 1, 1.5, 1.5, -1))
        ),
    ]
    rabis = np.array(
        [
...
def test_solve_custom_network_batched_vs_qutip(
    ...
    # Reference ground truth via QuTiP
    c0 = np.zeros((3, 3), dtype=complex)
    c0[0, 2] = np.sqrt(4.0e6)
    c1 = np.zeros((3, 3), dtype=complex)
    c1[1, 2] = np.sqrt(2.0e6)
...
def test_solve_quantum_ladder_parameter_sweeps_and_overrides_vs_qutip(
    tol_parity: float,
):
    ...
    atom = get_atom("Rb85")
    gamma_1 = 1.0 / atom.getStateLifetime(
        5, 1, 1.5, temperature=300.0, includeLevelsUpTo=80
    )
    ...
    qt_det_coherences = []
    for det in det_sweep:
        h_qt = build_ladder_hamiltonian(
            np.array([hz_to_rad_s(det)]), np.array([rabi_base])
        )
        rho_ss = solve_qutip_steady_state(h_qt, c_ops)
